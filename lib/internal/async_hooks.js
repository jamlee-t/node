...
// JAMLEE: 三个变量的解释 async_hook_fields async_id_fields async_ids_stack
...
/////////////////////////////////////////////////////////////////
//
// JAMLEE: pushAsyncIds 和 popAsyncIds 来自于 C++ 代码。是函数，其参数类型为 const FunctionCallbackInfo<Value>& args
// 存储一对 executionAsyncId and triggerAsyncId 到 async_ids_stack_。
//
/////////////////////////////////////////////////////////////////
...
// JAMLEE: active_hooks 中的属性用于跟踪正在运行的钩子集, 在启用/禁用另一个挂钩的情况下执行。 一旦活动的钩子集执行完毕，新的钩子集就会恢复。
...
// JAMLEE: 创建函数。这里 emitInitNative 是 js 定义的函数。
...
// JAMLEE: node 在 bootstrap 阶段时，调用 setupHook 设置 nativeHooks。
...
function emitInitNative(asyncId, type, triggerAsyncId, resource) {
  ...
  try {
    // JAMLEE: active_hooks.array 数组里装的是对象
    for (var i = 0; i < active_hooks.array.length; i++) {
      if (typeof active_hooks.array[i][init_symbol] === 'function') {
        // JAMLEE: 调用 active_hook.array 中的 初始化函数
...
// JAMLEE: 用于创建 emit*Native 函数。这里看到和 emitinitNative 方法不同，这里 hook 函数的参数只有 asyncId
...
// JAMLEE: 创建 emit*Native 函数，给函数bind传了2个参数: undefined, symbol, this 变量是 emitHook
// 这些函数是 setupHooks 用的。
// https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_objects/Function/bind
function emitHookFactory(symbol, name) {
  const fn = FunctionPrototype.bind(emitHook, undefined, symbol); // JAMLEE: this = emitHook, arg0 = undefined, arg1 = sysmbol
...
// JAMLEE: 从 active_hooks.array 中拷贝到 active_hooks.tmp_array
...
// JAMLEE: 从 tmp_array 中拷贝到 active_hooks.array
...
// JAMLEE: 激活的 hook。async_hook_fields[kCheck] 代表 1 个 AsyncHook 对象(里面是一组)
...
// JAMLEE: async_id_fields 存储了当前所有的异步id。
...
// JAMLEE: 获取某个对象是否有 async_id_symbol。没有则新建 1 个
...
// JAMLEE: 获取当前异步环境的默认的 defaultTriggerAsyncId。如果 defaultTriggerAsyncId 是 -1, 
// 那么从 async_id_fields[kExecutionAsyncId] 中取。
...
// JAMLEE: 清理 defaultTriggerAsyncId。
function clearDefaultTriggerAsyncId() {
  async_id_fields[kDefaultTriggerAsyncId] = -1;
}

// JAMLEE: 在 triggerAsyncId 中执行 block。也就是手动临时指定1个triggerAsyncId。
...
// JAMLEE: init hook 是否有1个或者多个
function initHooksExist() {
  return async_hook_fields[kInit] > 0;
}

// JAMLEE: after hook 是否有1个或者多个
function afterHooksExist() {
  return async_hook_fields[kAfter] > 0;
}

// JAMLEE: destroy hook 是否有1个或者多个
...
module.exports = {
  ...
  // JAMLEE: 用这 4 个方法能够封装 async 对象, JS 触发 async 的事件。底层调用 emitInitNative
  ...
  // JAMLEE: 用这 4 个方法能够封装 async 对象, C++ 触发 async 的事件。这里底层用的是 emitHook，会调用到 active_hooks.array 里面存储的函数。
  // tiveHooks 在 lib/internal/bootstrap/node.js 中的调用, 会设置到 C++ 的 env.async_hook 中
