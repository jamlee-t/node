...
// JAMLEE: 处理写入请求。写入任务在 libuv 中是一种 req?
function handleWriteReq(req, data, encoding) {
  const { handle } = req;

  // 调用底层 C++ 方法写入数据，writeLatin1String， writeUtf8String ....
  switch (encoding) {
    case 'buffer':
    {
...
// JAMLEE: nodejs 调用写入数据到 C++ 
function writeGeneric(self, data, encoding, cb) {
  const req = createWriteWrap(self[kHandle]);
  const err = handleWriteReq(req, data, encoding); // 处理写入请求。
...
////////////////////////////////////////////////////////
//
// JAMLEE: C++ 调用这个函数传回数据给 Js。Node 中 Stream 类的底层资源用这个 stream.push 数据给 Stream。
//
////////////////////////////////////////////////////////
function onStreamRead(arrayBuffer) {
  ...
  const handle = this;
  const stream = this[owner_symbol]; // 获取与当前 handle 相关的 stream(也就是node的stream)

  stream[kUpdateTimer]();

  // 当前读取的有数据且stream 不是销毁状态
  if (nread > 0 && !stream.destroyed) {
    ...
    if (userBuf) {
      ...
      result = stream.push(buf); // 在 stream 中 push 数据。
    }
    if (!result) {
...
// JAMLEE: stream 的额外工具类。本身不是 stream 的一份子。在 net.js 中为 socket 设置超时用到。
